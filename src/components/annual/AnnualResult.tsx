...
import { Briefcase, Heart, Activity, DollarSign, RotateCcw, Download, Share2 } from "lucide-react";
import ScoreRing from "./ScoreRing";
import MonthlyForecast from "./MonthlyForecast";
import { downloadPDF, shareImage } from "@/lib/export-report";
...
const AnnualResult = ({ data, onReset }: AnnualResultProps) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    toast.info("正在生成 PDF...");
    try {
      await downloadPDF(reportRef.current, `${data.targetYear}流年報告.pdf`);
  ...
  const handleShareImage = async () => {
    ...
    toast.info("正在生成長圖...");
    try {
      await shareImage(reportRef.current, `${data.targetYear}流年報告`);
      toast.success("圖片已生成！");
    } catch (e) {
      console.error(e);
      toast.error("圖片生成失敗，請稍後再試。");
    } finally {
      setExporting(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap justify-center gap-3 pb-8">
        <Button onClick={handleDownloadPDF} disabled={exporting} variant="outline" className="border-gold/40 text-gold hover:bg-gold/10">
          <Download className="w-4 h-4 mr-2" /> 下載 PDF
        </Button>
        <Button onClick={handleShareImage} disabled={exporting} variant="outline" className="border-gold/40 text-gold hover:bg-gold/10">
          <Share2 className="w-4 h-4 mr-2" /> 分享長圖
        </Button>
